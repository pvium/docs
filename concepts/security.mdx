...
Privy provides account and wallet infrastructure. Public blockchain networks record transfers. Pvium uses signed payout instructions to authorize payments and detect changes to those instructions.
...
Pvium requires signed payout instructions before funding. The following sections explain who signs those instructions, what they authorize, and how your team funds the payout.
...
After finalization, Pvium provides a funding link for the signed payout. The person responsible for your organization’s wallet opens the link, reviews the payout, and authorizes the funding transaction.
...
Use payout statuses, stable payout IDs, and failure details to track settlement and recover from errors:
...
Dashboard access and API credentials let users manage payout data. Finalization requires a signing key, and funding requires authorization from the funding wallet.
