...
Your organization retains control of its wallet and authorizes each payout before funds leave it.
...
Keeping funds outside Pvium removes the need for a provider-held balance. API permissions and payout key limits restrict what each credential can authorize.
...
<AccordionGroup>
  ...
  <Accordion title="Separate controls for accounts, API keys, and payout keys">
    Dashboard access alone does not authorize wallet transactions. API keys permit only the operations in their scopes. Payout keys can spend from their authorized pool within specified limits and until expiration. Protect the funding wallet and private signing keys because they authorize funding and payout finalization.
  </Accordion>
</AccordionGroup>
